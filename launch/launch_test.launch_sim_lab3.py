...
def generate_launch_description():
    ...
    stop_system = Node(
        package="wall_follower",
        executable="stop_system",
        parameters=[
            {"scan_topic": "/scan",
            "drive_topic": "/drive",
            "stop_topic": "/vesc/low_level/input/safety"}],
        name='stop_system',
        remappings=[
            ('/stop_system/pose', '/pose'),
            ('/stop_system/map', '/map'),
            ('/stop_system/base_link', '/base_link'),
            ('/stop_system/tf', '/tf'),
            ('/stop_system/tf_static', '/tf_static'),
        ]
    )
        ...
        RegisterEventHandler(
            OnProcessStart(
                ...
                on_start=[
                    ...
                    TimerAction(
                        period=1.0,
                        actions=[wall_follower, stop_system],
                    )
                ]
            )
        ),
